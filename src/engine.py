...
SCREEN_SIZE = game.getScreenSize()
CENTRE = game.getCentre()
OUTER_CIRCLE_DIM = game.getOuterCircleDim()
boundaryCircle = game.getBoundaryCircle()
BLOCKS = game.getBlocks()
BLOCKS_BY_INDEX = ["cyan", "purple", "magenta", "orange", "yellow", "green", "black"]
BLOCK_RECT = BLOCKS["cyan"].get_rect()
        ...
        for j in range(len(gameMatrix[i])):
                if len(gameMatrix[i][j]) <= 2:
                    if gameMatrix[i][j][0] == "null":
                        currentBlock = BLOCKS["black"]
                    else:
                        currentBlock = BLOCKS[gameMatrix[i][j][0]]
                else:
                    currentBlock = BLOCKS[gameMatrix[i][j]]
...
directionToFire = 0
game = Geometris()
INTERNAL_RADIUS = game.getInternalRadius()
CENTRE_CIRCLE_RADIUS = game.getCentreCircleRadius()
initGameMatrix = GameMatrix(INTERNAL_RADIUS, CENTRE_CIRCLE_RADIUS)
gameMatrix = initGameMatrix.getMatrix()
MATRIX_HEIGHT = initGameMatrix.getHeight()
